...
const list = async (req: Request, res: Response) => {
  ...
  if (Number.isNaN(limit)) {
    return res.status(400).send("limit가 숫자가 아닙니다.");
  }
  ...
  try {
    ...
    console.error(e);
    res.status(500).send("목록조회 시 오류가 발생했습니다.");
  }
};
...
const detail = async (req: Request, res: Response)  => {
  ...
  try {
    ...
    if (!result) return res.status(404).send("데이터가 존재하지 않습니다.");
    res.status(200).json(result);
  } catch(e) {
    console.error(e);
    res.status(500).send("상세조회 시 오류가 발생했습니다.");
  }
};
...
const create = async (req: Request, res: Response)  => {
  ...
  if (!title || !director || !year) return res.status(400).send("필수값이 입력되지 않았습니다.");
  ...
  try {
    ...
    console.error(e);
    res.status(500).send("등록 시 오류가 발생했습니다.");
  }
};
...
const update = async (req: Request, res: Response)  => {  
  ...
  try {
    ...
    console.error(e);
    res.status(500).send("수정 시 오류가 발생했습니다.");
  }
};
...
const remove = async(req: Request, res: Response)  => {
  ...
  try {
    ...
    console.error(e);
    res.status(500).send("삭제 시 오류가 발생했습니다.");
  }
};
